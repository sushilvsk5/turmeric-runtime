...
public class DeleteObjectFactoryInTypeLibraryTest extends
	...
	public void testGenTypeAddTypeDepOnComplexTypeSameLib() throws IOException {
		...
		TestResourceUtil.copyResource("types/CategoryProduct2.xsd", testingdir,
				"meta-src");

		// createTypeLibrary();

		String[] pluginParameter = { "-gentype", "genTypeAddType", "-pr",
				destDir.getAbsolutePath(), "-libname", "CategoryTypeLibrary",
				"-type", "CategoryProduct.xsd" };
		try {
		...
		String[] pluginParameter1 = { "-gentype",
				...
				destDir.getAbsolutePath(), "-libname", CATEGORY_TYPE_LIBRARY };
		...
		codegenAssertFileNotExists(destDir + File.separator + "gen-src",
				"org/ebayopensource/turmeric/common/v1/types/ObjectFactory.java");

		codegenAssertFileNotExists(destDir + File.separator + "gen-src",
				"org/ebayopensource/turmeric/common/v1/types/package-info.java");
	...
	private boolean createTypeLibrary(String pr) {
		...
		String[] pluginParameter = { "-gentype", "genTypeCreateTypeLibrary",
				"-pr", pr, "-libname", CATEGORY_TYPE_LIBRARY, "-libVersion",
				"1.2.3", "-libNamespace",

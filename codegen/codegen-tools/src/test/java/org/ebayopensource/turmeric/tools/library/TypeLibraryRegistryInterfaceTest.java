 ...
 * 
 */
public class TypeLibraryRegistryInterfaceTest extends
	...
	public static TypeLibraryType getTypeLibrary(String libraryName)
			throws Exception {
		TypeLibraryType typeLibrary = m_soaTypeRegistry
				.getTypeLibrary(m_libraryName);
		assertNotNull(
				"Unable to get TypeLibraryType for [" + libraryName + "]",
				typeLibrary);
		return typeLibrary;
	}
	...
	public void getAllTypeLibraries_forSize() throws Exception {
		...
		assertTrue(list.size() >= 1); // since the type library could have
										// already some libraries
	...
	public void getLibrariesReferredByType() throws Exception {
		...
		Set<String> set = m_soaTypeRegistry
				.getLibrariesReferredByType("harddiskType222");
	...
	public void getLibrariesReferredByType_overRiddenMethod() throws Exception {
		...
		Set<String> set = m_soaTypeRegistry
				.getLibrariesReferredByType(libraryType);
		Assert.assertThat(set, hasItem(m_libraryName));
	}
	...
	public void getDependentParentTypeFiles() throws Exception {
		...
		List<LibraryType> actualList = m_soaTypeRegistry
	...
	public void doesTypeNameExist_StringInput() throws Exception {
		Assert.assertThat(m_soaTypeRegistry.doesTypeNameExist("harddiskType"),
				is(true));
	}
	...
	@Test
	public void addTypeToRegistry_TypeInput_Fail_case_NS_mismatch()
			throws Exception {
		try {
			...
			Assert.assertThat(
					ex.getMessage(),
					allOf(containsString("The namespace of the type and the library to which it belongs to do not match"),
	...
	public void addDependencyToRegistry() throws Exception {
		String typeDependenciesFilePath = TypeLibraryConstants.META_INF_FOLDER
				+ "/" + m_libraryName + "/" + "NewTypeDependencies.xml";
		...
		int initialSize = m_soaTypeRegistry.getDependentParentTypeFiles(
				libraryType).size();

		InputStream inStream = null;
		try {
			inStream = myClassLoader
					.getResourceAsStream(typeDependenciesFilePath);
			typeLibraryDependencyType = JAXB.unmarshal(inStream,
					TypeLibraryDependencyType.class);

			for (TypeDependencyType type : typeLibraryDependencyType.getType()) {
				if (type.getName().equals(typeName)) {
					m_soaTypeRegistry.addDependencyToRegistry(type,
							m_libraryName);
				}
			}

			int finalSize = m_soaTypeRegistry.getDependentParentTypeFiles(
					libraryType).size();
	...
	@Test
	public void removalOfPartialDependencyFromTheDependencyFile()
			throws Exception {
		String typeDependenciesFilePath = TypeLibraryConstants.META_INF_FOLDER
				+ "/" + m_libraryName + "/" + "NewTypeDependencies.xml";
		ClassLoader myClassLoader = Thread.currentThread()
		...
		int initialSize = m_soaTypeRegistry.getDependentChildTypeFiles(
				libraryType).size();
		InputStream inStream = null;
		try {
			inStream = myClassLoader
			...
			int finalSize = m_soaTypeRegistry.getDependentChildTypeFiles(
					libraryType).size();
	...
	@Test
	public void removalOfTotalDependencyFromTheDependencyFile()
			throws Exception {
		...
		int initialNumberOfDirectParents = m_soaTypeRegistry
				.getDependentParentTypeFiles(libraryType, 1).size();
		...
		int initialNumberOfDirectChilds = m_soaTypeRegistry
				.getDependentChildTypeFiles(libraryType2, 1).size();

		String typeDependenciesFilePath = TypeLibraryConstants.META_INF_FOLDER
				+ "/" + m_libraryName + "/"
				+ "NewTypeDependenciesDependencyTotallyRemoved.xml";
		ClassLoader myClassLoader = Thread.currentThread()
				.getContextClassLoader();
		TypeLibraryDependencyType typeLibraryDependencyType = null;
		...
		try {
			...
			TypeDependencyParser typeDependencyParser = TypeDependencyParser
					.getInstance();
			typeDependencyParser
					.processTypeLibraryDependencyType(typeLibraryDependencyType);

			int finalNumberOfParents = m_soaTypeRegistry
					.getDependentParentTypeFiles(libraryType, 1).size();
			int finalNumberOfDirectChilds = m_soaTypeRegistry
					.getDependentChildTypeFiles(libraryType2, 1).size();

			boolean areParentsGood = initialNumberOfDirectParents > 0
					&& finalNumberOfParents == 0;
			boolean areChildsGood = ((initialNumberOfDirectChilds - finalNumberOfDirectChilds) == 1) ? true
					: false;
	...
	private boolean assertLibraryTypeLists(List<LibraryType> expectedList,
			List<LibraryType> actualList) {
